...
async def run_agent(
    ...
    *,
    ...
    if report_path is None:
        fallback_content = (
            f"# OSINT Report: {task}\n\n"
            f"## Summary\n{final_message or 'The agent ended without a final summary.'}\n\n"
            "## Notes\n"
            "The agent ended before calling save_report(), so this fallback report "
            "was generated by the runner."
        )
        try:
            paths = reporting.save_report(task, fallback_content)
            report_path = paths["markdown"]
        except Exception as exc:  # noqa: BLE001
            log.error("Could not save fallback report: %s", exc)
...
sources with normal web, DNS, HTTP and TLS requests; no exploitation,
intrusion or active port scanning. You are responsible for complying with
all applicable laws (e.g. GDPR) and each service's terms.
        """,
    )
